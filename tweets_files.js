...
exports.MyLittleTweets = function () {
	...
	var list = [];
	var filename = config.datapath() + 'tweetstore.json';
	...
	function saveMachine(voteuserid, userdata, callback) {
		fs.writeFile(config.datapath() + 'users/' + voteuserid + '-tweets-bayes.json', JSON.stringify(userdata.machine, null, '\t'), 'utf8', function (err) {
			if (err) {
	...
	function saveHuman(voteuserid, userdata, callback) {
		fs.writeFile(config.datapath() + 'users/' + voteuserid + '-tweets-human.json', JSON.stringify(userdata.human, null, '\t'), 'utf8', function (err) {
			if (err) {
	...
	me.initUser = function (user, callback) {
		...
		loadData(config.datapath() + 'users/' + user.id + '-tweets-human.json', function (data) {
			userdata.human = (data || {});
			loadData(config.datapath() + 'users/' + user.id + '-tweets-bayes.json', function (data2) {
