...
exports.Utils = function () {
    var me = this;
    var validChars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZïÆÐæàáäãåâÂÄÃçÇéèêøñíìîòöôõóÖÓüûúûùÜß';
    ...
    me.getCreatedHourOf = function (created_at) {
        return new Date(created_at.getFullYear(), created_at.getMonth(),
            created_at.getDate(), created_at.getHours(), 0, 0, 0).getTime();
    };

    me.getCreatedDayOf = function (created_at) {
        return new Date(created_at.getFullYear(), created_at.getMonth(),
            created_at.getDate(), 0, 0, 0, 0).getTime();
    };

    me.getDay = function (t) {
    ...
    me.storeJsonFile = function (list, filename) {
        var lines = list
            .map(function (t) {
                return (JSON.stringify(t));
            });
        if (lines.length) {
            console.log(filename, 'lines:', lines.length);
            fs.writeFileSync(filename, '[\n' + lines.join(',\n') + '\n]\n', 'utf8');
        }
    };

    me.storeJsonFileAsync = function (list, filename, cb) {
        var lines = list
            .map(function (t) {
                return (JSON.stringify(t));
            });
    ...
    me.loadDayJsonFiles = function (folder, cb) {
        var list = [];
    ...
    me.storeDayJsonFiles = function (list, folder, cb) {
        var min = 0, max = 0;
        ...
        var days = [];
        for (var key in dayso) {
            days.push({name: key == 'error' ? key : moment(new Date(parseInt(key))).format('YYYY-MM-DD'), tweets: dayso[key]});
        }
        var q = async.queue(function (o, callback) {
            me.storeJsonFileAsync(o.tweets, folder + 'tweets-' + o.name + '.json', callback);
        }, 1);
        q.drain = function () {
            cb();
        };
