...
exports.MyLittleCountStat = function () {
    ...
    function getCountData(params, callback) {
        ...
        params.store.enumerateVotesCatsRange(params.voteuserid,params.min,params.max, function (tweet) {
                if (tweet) {
                        data.count += 1;
                        data.cats[tweet.human] = (data.cats[tweet.human] || 0) + 1;
                        var newdata = tokenizer.getCreatedDayOf(new Date(tweet.created_at));
