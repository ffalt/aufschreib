...
var countsstat = require('./stats_counts').MyLittleCountStat();
...
exports.MyLittleStats = function () {
        ...
        getStat(params).getData(params, function (data) {
            fs.writeFile(file, JSON.stringify(data, null, '\t'), 'utf8', function (err) {
                if (err) {
                    ...
                    var statinfo = consts.tools.statinfo(params.type);
                    var text = consts.tools.modeName(params.mode).name + ' - ' + statinfo.name;
                    if (params.kind) {
                        var statkind = consts.tools.infokind(params.type, params.kind);
                        text += ' - ' + statkind.name;
                    }
                    logcb("Ansicht '" + text + "' gespeichert");
                }
                callback();
            });
        });
    ...
    me.cacheStats = function (voteuserid, store, logcb, callback) {
        ...
        writeCacheMode(params, logcb, function () {
            params.mode = 'machine';
            writeCacheMode(params, logcb, function () {
                logcb('Zwischenspeicher aktualisiert');
                callback();
            });
        });
    };
