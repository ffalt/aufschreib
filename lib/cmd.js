...
//            https://pbs.twimg.com/profile_images/1858287941/twitterpic_bigger.jpg
    ...
    function responseCmdCommands(req, res) {
        var params_machine = stats.getParams(req.user.id, store, 'pie', 'machine');
        var params_human = stats.getParams(req.user.id, store, 'pie', 'human');
        var users = null;
    ...
    function responseCmdClassify(req, res) {
        classifier.classify(req.user.id, store, console.log, function (isdone) {
                if (isdone) {
                    var params = stats.getParams(req.user.id, store, 'pie', 'machine');
    ...
    function responseChart(req, res, type, mode, cat, kind) {
        var start = +new Date();
        var params = stats.getParams(req.user.id, store, type, mode || consts.defaultmode, cat, kind);
        if (!params.isValid()) {
    ...
    function responseCmdJson(req, res, type, mode, cat, kind, min, max) {
        var start = +new Date();
        var params = stats.getParams(req.user.id, store, type, mode || consts.defaultmode, cat, kind, min, max);
        if (!params.isValid()) {
        ...
        if (!cmd) {
            ...
            switch (cmd) {
                ...
                case 'classifiction':
                    ...
                    break;
                case 'json':
                    responseCmdJson(req, res, query.type, query.mode, query.cat, query.kind, query.min, query.max);
                    break;
                default:
                    responseUnknown(res);
                    break;
                ...
                fs.readFile(file.path, function (err, data) {
                    ...
                    store.importHumanCats(id, data, function (err) {
                        if (err)
                            ...
                            var params = stats.getParams(id, store, 'pie', 'human');
                            res.render('stats/' + 'pie_commands', {
                                chartparams: params,
                                consts: consts,
                                hide_head: true,
            ...
            classifier.classify(socket.handshake.user.id, store,
                ...
                function (isdone) {
                    if (isdone) {
                        if (!socket.handshake)
                            return;
                        var params = stats.getParams(socket.handshake.user.id, store, 'pie', 'machine');
